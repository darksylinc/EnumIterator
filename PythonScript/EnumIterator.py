...
"""{0}::EnumData {0}::mSortedByEnum[{1}+1] =
{{
"""
...
{0}::EnumData {0}::mSortedByString[{1}+1] =
{{"""

cppEnumDeclaration = \
"""{0}::EnumData {0}::mSortedByEnum[4] =
{{
	{0}::EnumData( One,		"One" ),
	{0}::EnumData( Two,		"Two" ),
	{0}::EnumData( Three,		"Three" ),
	{0}::EnumData( Three,		"__INVALID" )
}};

{0}::EnumData {0}::mSortedByString[4] =
{{
	{0}::EnumData( One,		"One" ),
	{0}::EnumData( Three,		"Three" ),
	{0}::EnumData( Two,		"Two" ),
...
def processEnumFile( fullPath, fileName ):
	...
	for cppEnum in cppParsed.enums:
		...
		for enumValue, enumString in enumsByValue.items():
			outputStr += '{0}::EnumData( {1},		"{1}" )\n'.format( className, enumString )
		outputStr += '{0}::EnumData( {1},		"__INVALID" )\n'.format( className, enumsByString.keys()[-1] )

		outputStr += cppArrayDeclaration1.format( className, len( enumsByValue ) )

		for enumString, enumValue in enumsByString.items():
			outputStr += '{0}::EnumData( {1},		"{1}" )\n'.format( className, enumString )
		outputStr += '{0}::EnumData( {1},		"__INVALID" )\n'.format( className, enumsByString.keys()[-1] )
